...
每个变更在 `openspec/changes/<change-id>/.orbit-state` 维护一个 YAML 状态文件，记录当前阶段、子状态、任务进度、提交历史、回滚锚点。状态只在关键节点落盘（如 Stage 0 类型定稿后才写一次完整状态，不写半成品）。**对话断了？输入"继续"就能从断点恢复。**
...
Stage 4 每个 Task 独立 commit，并记录任务前 HEAD（`rollback_base`）作为回滚锚点。Review 失败时，`git reset --hard <rollback_base>` + `git clean -fd` 回滚到该 Task 开始前的稳定点，不污染 Git 历史（旧 commit 仍在 reflog）。整个实现阶段还有一个 `stage4_baseline_commit` 作为 Task 1 的兜底锚点。
...
所有子 Skill（含 Stage 5 的 `openspec-verify-change` / `openspec-archive-change` 以及 `openspec status/verify/archive` CLI）均有内联 fallback，断流风险低。
...
| **Stage 1** | Explore & Design | 头脑风暴，产出设计文档，定稿 change-id（智能放行：仍吻合则直通，范围漂移才改名） | feature |
| **Stage 2** | Specify | 方案分析（可选）→ 生成 OpenSpec 制品（docs 轻量：仅 proposal）→ **人工审阅 Gate** | 全部 |
...
Orbit:
  ...
  → 定稿 change-id（仍吻合则自动放行，范围漂移才会提议改名）
...
Orbit:
  ...
  → docs 轻量：默认仅写 proposal.md，跳过 design.md / specs
  → 🚧 人工审阅 Gate
...
Orbit 为每个子 Skill 提供了内联 fallback 指令（brainstorming、writing-plans、TDD、subagent-driven-development、systematic-debugging、verification、finishing-branch，以及 Stage 5 的 openspec-verify/archive）。即使 Superpowers 的某个 skill 文件不存在，Orbit 也能靠内置指令完成对应阶段。

**Q: 每次都要选变更类型、确认 Change ID 吗？**

不用。Stage 0 会按关键词智能分类——请求里明确出现"新增/实现"就自动判 feature、"修复/bug"判 bugfix、"文档/配置"判 docs，单类型匹配且无冲突时直接放行，不再弹选项。只有关键词冲突或无匹配时才让你选。Stage 1 的 Change ID 同理：brainstorming 后若 id 仍准确描述变更就自动放行，只有范围发生漂移时才提议改名。
